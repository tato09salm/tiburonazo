...
const CATEGORY_HIGHLIGHTS = [
  { name: "Competencia", slug: "jammer", emoji: "🏆", color: "from-blue-600 to-primary" },
  { name: "Entrenamiento", slug: "ropa-de-bano", emoji: "💪", color: "from-primary to-secondary" },
  { name: "Accesorios", slug: "lentes", emoji: "🥽", color: "from-secondary to-cyan-300" },
  ...
  { name: "Personalizados", slug: "personalizados", emoji: "✨", color: "from-purple-500 to-primary" },
];
...
export default async function HomePage() {
  ...
  return (
    <div>
      ...
      {/* Features strip */}
      <section className="bg-bg py-6 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {FEATURES.map(({ icon: Icon, label }) => (
              <div key={label} className="flex items-center gap-3 justify-center md:justify-start">
                <div className="w-10 h-10 rounded-xl bg-light flex items-center justify-center text-primary flex-shrink-0">
                  <Icon size={20} />
                </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/productos?featured=true" className="text-primary text-sm font-semibold hover:underline flex items-center gap-1">
      ...
      {/* Promo banner */}
      <section className="bg-gradient-to-r from-primary to-secondary text-white py-12 my-8">
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h2 className="font-display text-4xl font-bold">¿Buscas algo personalizado?</h2>
            <p className="mt-2 text-white/80">Uniformes de equipo, bordados, sublimados y más.</p>
          </div>
          <Link href="/categoria/personalizados" className="bg-white text-primary font-bold px-8 py-3 rounded-xl hover:shadow-lg transition-shadow flex-shrink-0">
            Cotizar ahora
          </Link>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-8 pb-16">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/productos" className="text-primary text-sm font-semibold hover:underline flex items-center gap-1">
            Ver todos <ArrowRight size={14} />
          </Link>
        </div>
